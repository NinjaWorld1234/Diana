...
const STATUS_STYLES: Record<string, any> = {
  LOCKED: { bg: 'var(--color-bg-hover)', border: 'var(--color-border)', opacity: 0.6, icon: Lock },
...
export default function ConceptMapPage() {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const getPosition = (i: number) => {
      ...
      // Arabic snake layout: starts from right to left on even rows, left to right on odd rows.
      const col = row % 2 === 0 ? (cols - 1) - (i % cols) : (i % cols);
      return { row, col };
    };
    ...
    const flowEdges: Edge[] = masteryMap.slice(1).map((n: any, i: number) => {
      const posA = getPosition(i);
      const posB = getPosition(i + 1);
      ...
      const strokeColor = n.status === 'COMPLETED' ? '#10B981' : n.status === 'IN_PROGRESS' ? '#3B82F6' : '#475569';
