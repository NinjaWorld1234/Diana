...
export default function NodePage() {
  ...
  const evaluateResults = async () => {
    const understanding = questionsByLevel.UNDERSTANDING.some((q: any) => answers[q.id]?.isCorrect);
    const application = questionsByLevel.APPLICATION.some((q: any) => answers[q.id]?.isCorrect);
    const reasoning = questionsByLevel.REASONING.some((q: any) => answers[q.id]?.isCorrect);
            ...
            {getCurrentQuestions().length > 0 ? (
              <div className="glass-card" style={{ padding: '32px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '24px' }}>
                  {getCurrentQuestions()[currentQ]?.options?.map((opt: any) => (
                    <button
                      key={opt.id}
                      onClick={() => !showFeedback && setSelectedOption(opt.id)}
                      style={{
                        padding: '14px 18px', borderRadius: 'var(--radius-sm)',
                        border: `2px solid ${selectedOption === opt.id ? 'var(--color-primary)' : showFeedback ? (opt.isCorrect ? '#10B981' : answers[getCurrentQuestions()[currentQ]?.id]?.selectedId === opt.id ? '#EF4444' : 'var(--color-border)') : 'var(--color-border)'}`,
                ...
                {!showFeedback ? (
                  <button className="btn-primary" onClick={handleSubmitAnswer} disabled={!selectedOption}>
